...
const SERVICES = [
  {
    ...
    desc: "送禮、投資、自己喝",
    emoji: "🍷",
    href: "/v4/banbun/chat?prompt=幫我找一支適合送禮的酒",
    disabled: false,
  },
  {
    key: "daily",
    ...
    desc: "生活雜貨，下單送到家",
    emoji: "🧴",
    href: "/v4/banbun/chat?prompt=我想買日用品",
    disabled: false,
  },
  {
    key: "bill",
...
export default function BanbunHomePage() {
  ...
  const submitHomeInput = () => {
    ...
    if (!text) return;
    router.push(`/v4/banbun/chat?prompt=${encodeURIComponent(text)}`);
  };
    ...
    <div className="relative flex flex-col overflow-hidden bg-white">
      {drawerOpen && (
        <HistoryDrawer
          ...
          onNewChat={() => router.push("/v4/banbun/chat?new=1")}
          onOpenConversation={(id) =>
            router.push(`/v4/banbun/chat?open=${id}`)
          }
          onOrders={() => router.push("/v4/orders")}
        />
      )}
          ...
          <div className="relative flex w-full flex-col overflow-hidden rounded-2xl bg-gray-800 px-5 pb-10 pt-5 text-white">
            ...
            <div className="relative z-10 mt-3 flex flex-col items-start gap-2">
              {chips.map((p) => (
                <Link
                  key={p}
                  href={`/v4/banbun/chat?prompt=${encodeURIComponent(p)}`}
