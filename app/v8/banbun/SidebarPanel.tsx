...
// 簡化成純 icon + 文字的清單樣式（照參考圖：無底色、無邊框、無箭頭，
// 圖示統一深色，列與列之間留較大間距），拿掉先前的彩色圓點卡片樣式
const ICON_META: Record<string, { icon: React.ReactNode }> = {
  "ai-select": { icon: <Icon src="/icons/tab-ai-select.svg" className="size-6" /> },
  "wine-select": { icon: <Icon src="/icons/tab-wine-select.svg" className="size-6" /> },
  "reward-marketplace": { icon: <Icon src="/icons/acc-gift.svg" className="size-6" /> },
  "rate-forecast": {
    icon: (
    ...
    <div className="relative flex h-full flex-col bg-white">
      ...
      <div className="no-scrollbar flex flex-1 flex-col gap-1 overflow-y-auto px-4 pb-2">
        ...
        {SERVICE_POOL.map((s) => {
          ...
          return s.disabled ? (
            <div key={s.key} className="flex items-center gap-4 px-1 py-3.5 text-left">
              <span className="shrink-0 text-gray-300">{meta.icon}</span>
              <p className="flex-1 text-[16px] font-semibold text-gray-400">
                {s.label}
              </p>
            ...
            <Link
              key={s.key}
              href={s.href}
              className="flex items-center gap-4 px-1 py-3.5 text-left active:opacity-60"
            >
              <span className="shrink-0 text-gray-800">{meta.icon}</span>
