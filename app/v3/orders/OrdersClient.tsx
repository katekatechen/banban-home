...
export default function OrdersClient() {
  ...
  // 帳號頁的「歷史交易紀錄」連進來不帶 source，看全部；側邊欄的「伴伴訂單紀錄」
  // 帶 source=伴伴對話，只看透過伴伴對話下的單——兩個入口內容不一樣，不是重複。
  const sourceFilter = searchParams.get("source");
  const pageTitle = sourceFilter === "伴伴對話" ? "伴伴訂單紀錄" : "歷史交易紀錄";
  const [orders, setOrders] = useState<Order[]>([]);
  const [tab, setTab] = useState<"訂單" | "詢問">("訂單");
  const [filter, setFilter] = useState<Filter>(
    (searchParams.get("filter") as Filter) ?? "全部",
  );

  useEffect(() => {
    const all = getOrders();
    setOrders(sourceFilter ? all.filter((o) => o.source === sourceFilter) : all);
  }, [sourceFilter]);
